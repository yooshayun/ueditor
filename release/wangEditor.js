...
Link.prototype = {
    ...
    _createPanel: function _createPanel(text, link) {
        ...
        document.querySelector('#' + btnOkId).addEventListener('click', function (e) {
            e.stopPropagation();
            var _text = document.querySelector('#' + inputTextId).value;
            var _link = document.querySelector('#' + inputLinkId).value;
            if (type == 0) {
                //添加文本链接
                _this._insertLink(_text, _link);
            } else if (type == 1) {
                //添加卡片链接
