    ...
    return rollup.rollup({
        ...
        bundle.write({
            ...
            gulp.src('./release/wangEditor.js')
                ...
                .pipe(gulpReplace(/-CONTAINT-CSS-/gm, function() {
                    var filePath = path.resolve(__dirname, 'release', 'content.min.css');
                    var content = fs.readFileSync(filePath).toString('utf-8');
                    // 替换 \n \ ' 三个字符
                    content = content.replace(/\n/g, '').replace(/\\/g, '\\\\').replace(/'/g, '\\\'')
                    return content
...
// 默认任务配置
gulp.task('default', () => {
    gulp.run('copy-fonts', 'css', 'contentcss', 'script')
